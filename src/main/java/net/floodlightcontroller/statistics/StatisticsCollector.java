				...
				for (OFStatsReply r : e.getValue()) {
					...
					for (OFPortStatsEntry pse : psr.getEntries()) {
						...
						if (portStats.containsKey(npt) || tentativePortStats.containsKey(npt)) {
							...
							long bw = spb.getBitsPerSecondRx().getValue();

							if(bw> PortRxThreshold) {
								log.info("RX: Se ha superado el umbral -> BW:{}",bw,">RX:{}",PortRxThreshold);
							} else if (bw> PortTxThreshold) {
								log.info("TX: Se ha superado el umbral -> BW:{}",bw,"<RX:{}",PortRxThreshold);
							} else if (bw< PortRxThreshold) {
								log.info("RX: No se ha superado el umbral -> BW:{}",bw,">RX:{}",PortRxThreshold);
							}else if (bw< PortTxThreshold) {
								log.info("TX: No se ha superado el umbral -> BW:{}",bw,"<RX:{}",PortRxThreshold);
							}
